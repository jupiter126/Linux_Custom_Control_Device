...
if [ ! -f  $directory/K90_Layout_$activelayout ]; then
	echo $(ls $directory/ |grep "K90_Layout_"|grep -v "~"|head -n1|cut -f 3 -d"_") > $directory/activelayout #If active layout is set to a non existing one, set it to an existing one
fi
source $directory/K90_Layout_$activelayout && $include $directory/K90_Layout_$activelayout # This is where we set the activelayout, load its variables and functions
...
if [ $(whoami) = "root" ]; then
	if [ "$gscpid" = "" ] || [ "$1" = "reset" ]; then #restart getscancodes
		fonction="restart_getscancodes"
		f_debug $fonction
		killall -q getscancodes # Modifications made, it's better if it works without killing ;)
		$directory/getscancodes $eventnbr | tee /dev/null $cachefile & # Somehow when I > the output in a file, I can't tail it without stopping getscancodes, although it doesn't stop when pipng trough tee
	fi
fi
}
f_initcache
...
fonction=f_switch
f_debug $fonction
# You can have as much custom layouts as you wish, each one should be a copy of this script, with as filename "K90_Layout_[youchangethispart]" f_switch automatically detects them and parses them sequentially one by one.
if [ ! -f $directory/activelayout ]; then #create state file if it doesn't exist
	if [ "$(ls $directory|grep "K90_Layout_"|grep -v "~")" = "" ]; then
		f_libnotify 5000 "FATAL" "No Layout file detected, please fix by putting a K90_Layout_* file"
		exit
	else
	echo "$(ls $directory|grep "K90_Layout_"|grep -v "~"|head -n "1"|tail -n 1|cut -f3 -d "_")" > $directory/activelayout
	fi
fi
n=$(ls $directory/ |grep "K90_Layout_"|grep -v "~"|wc -l) # scans $directory for "K90_script-" --> count configs
o=$(ls $directory|grep "K90_Layout_"|grep -v "~"|cut -f3 -d "_"|grep -ne "$(cat $directory/activelayout)"|cut -f1 -d ":") #get current conf number
if [ "$o" -ge "$n" ]; then
	echo "$(ls $directory|grep "K90_Layout_"|grep -v "~"|head -n "1"|tail -n 1|cut -f3 -d "_")" > $directory/activelayout
else
	o=$[ ( $o + 1 ) ]
	echo "$(ls $directory|grep "K90_Layout_"|grep -v "~"|head -n "$o"|tail -n 1|cut -f3 -d "_")" > $directory/activelayout
fi
...
fonction=f_list
f_debug $fonction
echo "$(ls $directory/ |grep "K90_Layout_"|grep -v "~"|wc -l) Layouts found:"
ls $directory/ |grep "K90_Layout_"|grep -v "~"
exit
}
