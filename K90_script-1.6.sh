#!/bin/bash
# dev release - not stable !!! so far, use 1.0
...
fonction=f_initcache
f_debug $fonction
#f_libnotify 12000 "CheatSheet for $layout layout" "$(cat $(ls -a|grep "$layout" |cut -f2 -d">")|grep "_-_"|grep -v cheatsheet|cut -f2 -d"=")"
f_libnotify 12000 "CheatSheet for $layout layout" "$(cat $(ls -a|grep "$layout" |grep -v "~"|cut -f2 -d">")|grep "_-_"|grep -v cheatsheet|cut -f2 -d"=")"
}
# f_initcache # was used here... to be replaced
...
if [ "$layout" = "Admin" ]; then #If launched from cron (layout=admin) then read last action and repeat 4 times on 1 min
